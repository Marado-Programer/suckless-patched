...
copyurl(const Arg *arg) {
	/* () and [] can appear in urls, but excluding them herewill reduce false
	...
	static const char* URLSTRINGS[] = {"http://", "https://", "ftp://", "rsync://", "gopher://", "gemini://"};
	...
	int i = 0, row = 0, col = 0, startrow = 0, colend = 0, passes = 0; /* row of current URI // column of current URI start // row of las occurrence // column of last occurrence // how many rows have been scanned */
	char *linestr = calloc(term.col+1, sizeof(Rune)); /* assume ascii */
	 ...
	 * next occurrance of a URI
	 */
	while(passes != term.bot + 2) {
		/* Read in each column of every row until
		 * we hit previous occurrance of URI
		 */
	...
	do{
		...
		if ((match = strstr(linestr, "http://")) ||
				...
				(match = strstr(linestr, "rsync://")) ||
 				(match = strstr(linestr, "gopher://")) ||
 				(match = strstr(linestr, "gemini://"))) 
			break;
		if (--row < term.top)
			row = term.bot;
	} while (row != startrow); 
