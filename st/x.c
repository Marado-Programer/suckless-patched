...
xloadsparefont(FcPattern *pattern, int flags)
{
	...
	/* Belive U+0000 glyph will present in each default font */
...
xinit(int cols, int rows)
{
	...
	xw.netwmname = XInternAtom(xw.dpy, "_NET_WM_ICON_NAME", False);
...
xdrawcursor(int cx, int cy, Glyph g, int ox, int oy, Glyph og, Line line, int len)
{
	...
	if (IS_SET(MODE_FOCUSED)) {
		...
		case 6: /* Steady bar */
			...
			break;
		case 7: /* Blinking snowman */
			if (IS_SET(MODE_BLINK))
				break;
			/* FALLTHROUGH */
		case 8: /* Steady snowman */
			g.u = stcursor;
			xdrawglyph(g, cx ,cy);
			break;
		}
