...
struct Client {
	...
	int bw, oldbw;
	unsigned int tags;
	int isfixed, iscentered, isfloating, isalwaysontop, isurgent, neverfocus, oldstate, isfullscreen, needresize, CenterThisWindow;
	Client *next;
	Client *snext;
	Monitor *mon;
	Window win;
...
toggleextrabar(const Arg *arg)
{
	...
	updatebarpos(selmon);
	XMoveResizeWindow(dpy, selmon->extrabarwin, selmon->wx, selmon->eby, selmon->ww + 2, bh);
	arrange(selmon);
}
